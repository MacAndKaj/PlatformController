...
SpiProxy::SpiProxy(const std::string& device_path)
    : m_device_path(device_path)
{
    ...
    {
        int err_status = errno;
        std::cout   << "Call ioctl(SPI_IOC_WR_MODE) failed with errno: (" 
                    << err_status << ") "
                    << std::strerror(err_status) << std::endl;
        close(m_fd);
    ...
    {
        int err_status = errno;
        std::cout   << "Call ioctl(SPI_IOC_WR_BITS_PER_WORD) failed with errno: (" 
                    << err_status << ") "
                    << std::strerror(err_status) << std::endl;
        close(m_fd);
        throw std::runtime_error("SpiProxy error");
    ...
    {
        int err_status = errno;
        std::cout   << "Call ioctl(SPI_IOC_WR_MAX_SPEED_HZ) failed with errno: (" 
                    << err_status << ") "
                    << std::strerror(err_status) << std::endl;
        close(m_fd);
        throw std::runtime_error("SpiProxy error");
...
SpiProxy::~SpiProxy()
{
    ...
    std::cout << "Closing SpiProxy" << std::endl;
}
